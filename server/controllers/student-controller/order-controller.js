...
const createOrder = async (req, res) => {
  try {
    ...
    const numericPrice = Number(coursePricing);

    if (Number.isNaN(numericPrice)) {
      return res.status(400).json({
        success: false,
        message: "Invalid coursePricing. Must be a number.",
      });
    }
    ...
    // Fallback: if PayPal creds are missing in dev, simulate approval URL
    if (!hasPaypalCreds) {
      ...
      const approveUrl = `${clientUrl}/payment-return?orderId=${newlyCreatedCourseOrder._id}&simulated=true`;
